...
func processTask(ctx context.Context, svcCtx *svc.ServiceContext, task InterviewTask) {
	logx.Infof("Processing interview task asynchronously for session: %s", task.SessionId)
	...
	if task.NextQuestion != "" {
		...
		if err != nil {
			logx.Errorf("Consumer failed to save interviewer dialogue: %v", err)
		}
	}
	...
	if userResp != nil && userResp.Id != "" {
		go func(dialogueId string) {
			...
			for idx, turn := range task.History {
				...
				historyBuilder.WriteString(fmt.Sprintf("  面试官提问: %s\n", turn.Question))
				historyBuilder.WriteString(fmt.Sprintf("  候选人回答: %s\n", turn.Answer))
			}
			...
			if svcCtx.Config.Mock {
				...
				commentary = `{
					...
					"overall_comment": "候选人语言组织能力优秀，技术点交代清楚，表现良好。"
				}`
			} else {
