...
import (
	...
	"ai_companion/companion_api/internal/config"
	"ai_companion/companion_api/internal/handler"
...
var configFile = flag.String("f", "etc/companion-api.yaml", "the config file")
...
func main() {
	...
	if c.RealtimeCompanion.Gemini.ApiKey == "" {
		...
		key := c.RealtimeCompanion.Gemini.ApiKey
		masked := key
		if len(key) > 8 {
			masked = key[:4] + "..." + key[len(key)-4:]
		}
	...
	server.AddRoute(rest.Route{
		...
		Path:    "/ws/companion",
		Handler: handler.CompanionWSHandler(ctx),
	})
